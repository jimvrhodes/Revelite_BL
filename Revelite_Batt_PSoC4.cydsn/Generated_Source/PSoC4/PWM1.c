...
#include "PWM1.h"
...
#if defined(PWM1__PC)
    #define PWM1_SetP4PinDriveMode(shift, mode)  \
    do { \
        PWM1_PC =   (PWM1_PC & \
                                (uint32)(~(uint32)(PWM1_DRIVE_MODE_IND_MASK << \
                                (PWM1_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (PWM1_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define PWM1_SetP4PinDriveMode(shift, mode)  \
        do { \
            PWM1_USBIO_CTRL_REG = (PWM1_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(PWM1_DRIVE_MODE_IND_MASK << \
                                    (PWM1_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (PWM1_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		PWM1_SetP4PinDriveMode(PWM1__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(PWM1_DR & (uint8)(~PWM1_MASK));
    drVal = (drVal | ((uint8)(value << PWM1_SHIFT) & PWM1_MASK));
    PWM1_DR = (uint32)drVal;
}
...
{
    return (uint8)((PWM1_PS & PWM1_MASK) >> PWM1_SHIFT);
}
...
{
    return (uint8)((PWM1_DR & PWM1_MASK) >> PWM1_SHIFT);
}
...
void PWM1_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  PWM1_INTCFG & (uint32)(~(uint32)position);
    PWM1_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(PWM1_INTSTAT & PWM1_MASK);
	PWM1_INTSTAT = maskedStatus;
    return maskedStatus >> PWM1_SHIFT;
}
